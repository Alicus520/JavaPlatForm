...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		str += '<img id="i' + this.obj + nodeId + '" src="' + ((node._io) ? node.iconOpen : node.icon) + '" alt="" border=0px />';		
	}
	if (node.url) {
		str += '<a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" href="' + node.url +'&nodeId='+ node.id +'&nodename='+ encodeURI(encodeURI(node.name)) +'"';		
