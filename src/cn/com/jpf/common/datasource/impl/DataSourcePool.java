...
import cn.com.jpf.common.datasource.IDataSourcePool;
...
import cn.com.jpf.common.utils.XMLUtil;

public class DataSourcePool implements IDataSourcePool {
	...
	private void createConnections(int numConnections) throws SQLException {
		...
		for (int x = 0; x < numConnections; x++) {
			...
			try{
				...
				throw new JpfException(e.getMessage(), JpfException.INFTYPE_ERROR);
			}
	...
	private Connection newConnection() throws SQLException {
		...
		if(StringUtil.isNotEmpty(dbFlag)){
			...
			try {
				...
				IDataSourceConnection dbConn = (IDataSourceConnection)cls.newInstance();
			...
			if (!pConn.isBusy()) {
				...
				if (!testConnection(conn)) {
					...
					try{
						...
						logger.debug(MsgManager.getInstance()
								.getLocaleString("message.jdbc.createConnection.Failured")); 
	...
	public void returnConnection(Connection conn) {
		...
		if (connections == null) {
			logger.error(MsgManager.getInstance()
					.getLocaleString("message.jdbc.connectionpool.isNull"));
			return;
		}
	...
	*/
	public synchronized void refreshConnections() throws SQLException {
		...
		// 确保连接池己创新存在
	  	if (connections == null) {
	  		logger.debug(MsgManager.getInstance()
	  				.getLocaleString("message.jdbc.connectionpool.isNullCannotRefresh"));
	  		return;
	  	}
	...
	*/
	...
	public synchronized void closeConnectionPool() throws SQLException {
		...
		// 确保连接池存在，如果不存在，返回
	  	if (connections == null) {
	  		logger.error(MsgManager.getInstance()
	  				.getLocaleString("message.jdbc.connectionpool.isNullCannotClose"));
	  		return;
	 	}
	...
	*/
	private void closeConnection(Connection conn) {
		try {
			...
			logger.error(MsgManager.getInstance()
					.getLocaleString("message.jdbc.connection.closeError"));
			throw new JpfException(e.getMessage());
		}
	}
